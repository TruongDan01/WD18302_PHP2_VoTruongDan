...
<script src="<?= _WEB_ROOT ?>../resources/assets/admin/js/pages/add-course.js"></script>
...
<script>
    ...
    if (document.getElementById('choices-teacher')) {
        var element = document.getElementById('choices-teacher');
        const example = new Choices(element, {
            removeItemButton: true
        });
...
$msg_error = Session::flash('msg-error') ?? '';
if (!empty($msg_error)) {
    ?>
    <script>
        Swal.fire({
            icon: 'error',
            title: '<?php echo $msg_error; ?>',
            toast: true,
            position: 'top-right',
            showConfirmButton: false,
            timer: 3000,
